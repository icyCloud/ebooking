...
class OrderModel(Base):
    ...
    is_pay = Column('isPay', TINYINT(4), nullable=False, default=0)
    exception_info = ''
    ...
    @classmethod
    def search(cls, session, merchant_id, hotel_id=None, id=None, hotel_name=None, checkin_date_start=None,
               checkin_date_end=None, customer=None, status=None, create_time_start=None, create_time_end=None,
               start=None, limit=None):
        if id:
            order = cls.get_by_id(session, id, hotel_id)
            if order:
                return [order], 1
            else:
                return [], 0
        query = session.query(OrderModel).order_by(OrderModel.id.desc()) \
            .filter(OrderModel.merchant_id == merchant_id)

        if hotel_id:
            query = query.filter(OrderModel.hotel_id == hotel_id)
        elif hotel_name:
    ...
    @classmethod
    def search_export_orders(cls, session, merchant_id, hotel_id=None, id=None, hotel_name=None, checkin_date_start=None,
               checkin_date_end=None, customer=None, status=None, create_time_start=None, create_time_end=None,
               order_ids=None):
        if id:
            order = cls.get_by_id(session, id, hotel_id)
    ...
    @classmethod
    def search_switch(cls, session, merchant_id, update_time=None, start=None, limit=None, order_id=None, status=None):
        query = session.query(OrderModel).order_by(OrderModel.id.desc()) \
            .filter(OrderModel.merchant_id == merchant_id)

        if order_id:
            query = query.filter(OrderModel.main_order_id == order_id)
        if update_time:
            date = datetime.datetime.strptime(update_time, "%Y-%m-%d %H:%M:%S")
            query = query.filter(OrderModel.update_time >= date)
        ...
        if status is not None:
            ...
            query = query.filter(OrderModel.status.in_([100, 300]))
    ...
    @classmethod
    def get_success_order_by_checkout_date_in_month(cls, session, merchant_id, year, month, pay_type, ota_ids=None):
    ...
    @classmethod
    def get_by_id(cls, session, id, hotel_id=None):
        query = session.query(OrderModel).filter(OrderModel.id == id)
        if hotel_id:
            query = query.filter(OrderModel.hotel_id == hotel_id)
        return query.first()

    @classmethod
    def get_by_merchant_and_id(cls, session, merchant_id, id, hotel_id=None):
        query = session.query(OrderModel).filter(OrderModel.id == id) \
            .filter(OrderModel.merchant_id == merchant_id)
        if hotel_id:
            query = query.filter(OrderModel.hotel_id == hotel_id)
    ...
    def get_by_main_order_id(cls, session, main_order_id):
        order = session.query(OrderModel).filter(OrderModel.main_order_id == main_order_id).first()
        return order

    @classmethod
    def change_order_status_by_main_order_id(cls, session, main_order_id, status):
        session.query(OrderModel).filter(OrderModel.main_order_id == main_order_id).update({'status': status})
        session.commit()

    @classmethod
    def change_order_pay_by_main_order_id(cls, session, main_order_id):
        session.query(OrderModel).filter(OrderModel.main_order_id == main_order_id).update({OrderModel.is_pay: 1})
        session.commit()
    ...
    @classmethod
    def get_waiting_orders(cls, session, merchant_id, start=None, limit=None, hotel_id=None):
        query = session.query(OrderModel).order_by(OrderModel.id.desc()) \
            .filter(OrderModel.merchant_id == merchant_id) \
    ...
    @classmethod
    def get_waiting_orders_count(cls, session, merchant_id, hotel_id=None):
    ...
    @classmethod
    def get_today_book_orders(cls, session, merchant_id, start=None, limit=None, hotel_id=None):
    ...
    @classmethod
    def get_today_checkin_orders(cls, session, merchant_id, start=None, limit=None, hotel_id=None):
        today = datetime.date.today()
